...
resolution quality is approximately 3.5$\times$ larger when 75\% of conflicts cross
...
information---demands the most coordination. Malone and
Crowston~\cite{malone1994interdependence} frame coordination as the
management of dependencies among activities, providing the vocabulary
for the dependency structures our experiments manipulate. Our
experimental design operationalizes this by varying the proportion of
conflicts that require cross-agent information.
...
existing test suites (\cref{tab:prelim}).
...
requires a structural condition absent in code-level tasks.
Experiment~3 tests this hypothesis directly. We
...
stakeholders (\cref{tab:stakeholders}) hold private constraint sheets
containing hard constraints, preferences, and hidden dependencies. Stakeholders are simulated by
...
(\cref{fig:architecture}). The structural difference is how
cross-agent information flows.
...
The six stakeholders form a conflict graph with natural clusters
(\cref{fig:partitions}). By reassigning stakeholders across the agent
boundary, we vary how many of the 8 conflicts require cross-agent
information.
...
\Cref{tab:matrix} summarizes the experimental matrix.
...
dimensions using a 1--5 scale (maximum 30):
...
(\cref{fig:composite-boxplot}): the layers that require
cross-stakeholder information synthesis. The largest effect is in L3 (Resolution Quality, \deff{+1.04},
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    L3         & $+0.10$ & $+0.36$ & $+0.25$ & 0.129\dmsig \\
    \bottomrule
  \end{tabular}
  \par\smallskip
  {\footnotesize Interaction $p$ from 100{,}000-permutation test.
  \dmsig$p < 0.15$, directionally consistent with H2.}
\end{table}

The L3 dose-response pattern is striking (\cref{fig:dose-response}):
the treatment advantage for resolution quality is approximately
3.5$\times$ larger in Partition~C ($+0.36$) than
...
\begin{figure}[t]
  ...
  \caption{Dose-response gradient. The treatment advantage in L3
    (Resolution Quality) is approximately $3.5\times$ larger in Partition~C
...
\Cref{fig:l3-breakdown} shows the per-run L3 scores.
...
in 5/8 runs). One run (Treatment-C-5, L3~=~0.21) is a clear failure---four
of eight conflicts scored ``missing''---falling well below both control
means (Control-A: 0.37, Control-C: 0.44). The remaining two runs
(L3~=~0.67 and 0.75) are degraded relative to treatment peers but still
above both control means. The hub-and-spoke topology trades ceiling for
consistency.
...
\begin{figure}[t]
  ...
  \caption{L3 (Resolution Quality) per-run breakdown. Treatment-C
    ...
    the highest variance, with one clear failure (Treatment-C-5,
    L3~=~0.21) alongside five runs above 0.88.}
  \label{fig:l3-breakdown}
\end{figure}

\subsection{Blind Architectural Review}

The blind review confirms the rubric's direction (\cref{tab:blind}).
Treatment documents score $23.9 \pm 2.2$ out of 30, compared to
$22.3 \pm 1.2$ for control ($\Delta = +1.7$).
...
The rubric and blind review measure different qualities
(\cref{fig:blind-vs-rubric}) and pick different individual champions
...
Three communication metrics (\cref{tab:comms}) provide mechanistic
evidence for why the peer topology outperforms. \emph{Relay transparency} measures
...
relay similarity 0.10--0.42), suggestive of the quality--fidelity
relationship predicted by H3, though the sample ($n = 4$) precludes
formal testing.
...
Treatment runs take 60--75\% longer (\cref{tab:process}) and involve
roughly twice the stakeholder interviews. This additional process is
the mechanism, not
...
(approximately 3.5$\times$ larger advantage at 75\% cross-partition vs.\ 25\%).
...
All data, code, and experiment protocols supporting the findings of this study are publicly available on GitHub.
The umbrella repository containing the paper and cross-experiment analysis is at \url{https://github.com/kar-ganap/ate-series}.
Individual experiment repositories contain raw run data (session transcripts, architecture documents, stakeholder constraint sheets), evaluation outputs (rubric scores, blind review scores), analysis scripts, and execution protocols:
Experiment~1 (bug-fixing): \url{https://github.com/kar-ganap/ate};
Experiment~2 (feature implementation): \url{https://github.com/kar-ganap/ate-features};
Experiment~3 (architecture design): \url{https://github.com/kar-ganap/ate-arch}.
The stakeholder simulation prompts and ground-truth rubrics used for evaluation are included in the Experiment~3 repository.

\section*{Funding and Competing Interests}
This research received no external funding. The author declares no competing interests. This work was conducted independently and is not affiliated with or sponsored by Anthropic or any other organization whose products are studied.
